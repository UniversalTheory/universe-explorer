...
| T29 | The galaxy model's 300k points are generated in a **Web Worker** (`galaxy.worker.ts` around the DOM-free `galaxy-gen.ts`) and requested once the first frames are out, or when the camera passes 1e14 km from the Sun (2026-09-20). | Generating inline cost 122 ms of the main thread at boot (`new Universe` 175 ms → 53 ms, measured), for points nobody can see until 1e15 km. Deferring without a worker would have moved the stall into the zoom-out instead. The worker bundles to 5 kB because the generator imports no Three.js. |
| T30 | The **exoplanet catalogue is fetched after boot**, not during `Ephemeris.load` (2026-09-20). Search, the PSR B1257+12 retype and `#planet` deep links catch up when it lands. | 1.06 MB (JSON + binary) that nothing in the Solar System view needs. Registration still happens within a second of boot, so search is complete in practice. |
| T31 | The events Web Worker imports its static tables from `src/data/event-tables.ts`, never `catalog.ts` (2026-09-20). | Importing the body catalogue pulled the curated star and exoplanet catalogues into the worker bundle: 141 kB → 70.6 kB once split. |
...
| P16 **(user)** | Stage G approved on 2026-09-20, **performance first**: galaxy generation off the boot path, exoplanets fetched lazily, the events worker freed of the body catalogue. Visual, UI, docs/CI and device QA remain open. | Owner chose the measurable items before the subjective ones. |
